...
spec = importlib.util.spec_from_file_location('hf_setup', project / 'scripts/setup.py')
...
try:
    ...
    try: setup.Setup(ipc, Picker(b, 'cancel')).prepare(a)
    except setup.Cancelled: pass
    else: raise AssertionError('Tiling confirmation must be cancellable')
    assert all(ipc.windows()[w]['floating'] and ipc.windows()[w]['at'] == before[w]['at'] for w in (a,b))
    assert not protect(1)
    passed('cancelling the tiling offer preserves floating apps and leaves no reservation')

    flow = setup.Setup(ipc, Picker(b, 'tile')); selected = flow.prepare(a)
    flow.apply(selected)
    ...
    assert protect(1) and ipc.status()['containers'][0]['faces'] == [[a],[b]]
    assert all(not ipc.windows()[w]['floating'] for w in (a,b))
    ...
    assert all(not ipc.windows()[w]['floating'] and ipc.windows()[w]['workspace']['id'] == 8 for w in (a,b))
    ipc.call('reload'); assert not ipc.call('configerrors')
    ...
    before = deepcopy(ipc.windows())
    flow = setup.Setup(ipc, Picker('workspace:9', c, 'create', 'tile'))
    selected = flow.prepare(a)
    real_action = ipc.action
    def reject_pair(command):
        if command == 'pair': raise setup.SetupError('Injected pair failure')
        return real_action(command)
    ipc.action = reject_pair
    ...
    for w in (a,c):
        ...
        assert any(t.rstrip('*') == 'chillmode' for t in current['tags'])
    ...
    real_action('reserve 8 expiry-test 1'); assert protect(8)
