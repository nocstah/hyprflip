#!/usr/bin/env python3
"""Shared card workflows with a replaceable choice/input/progress frontend."""
import argparse
import configparser
...
class Hyprctl:
    ...
    def save_duration(self, value):
        if type(value) is not int or not 0 <= value <= 2000:
            raise SetupError('Choose a duration between 0 and 2000 milliseconds.')
        env = self.env if self.env is not None else os.environ
        root = Path(env.get('XDG_STATE_HOME', str(Path.home() / '.local/state'))) / 'hyprflip'
        root.mkdir(parents=True, exist_ok=True)
        path = root / 'duration_ms'
        previous = path.read_bytes() if path.exists() else None
        old = self.data('-j', 'getoption', 'plugin:hyprflip:duration_ms')['int']
        if type(old) is not int or not 0 <= old <= 2000:
            raise SetupError('The current duration could not be read.')
        temporary = path.with_name('duration-' + uuid.uuid4().hex)
        try:
            temporary.write_text(str(value) + '\n')
            temporary.replace(path)
            self.call('eval', 'hl.config({plugin={hyprflip={duration_ms=%d}}})' % value)
            if self.data('-j', 'getoption', 'plugin:hyprflip:duration_ms')['int'] != value:
                raise SetupError('The duration could not be applied.')
        except Exception:
            if previous is None: path.unlink(missing_ok=True)
            else:
                temporary.write_bytes(previous); temporary.replace(path)
            try: self.call('eval', 'hl.config({plugin={hyprflip={duration_ms=%d}}})' % old)
    ...
    def tile(self, window):
        ...
        if chilled:
            self.call('eval', f'assert(chillmode and chillmode.handoff and chillmode.handoff("{address}"), '
                      '"Chill mode needs the Hyprflip integration before this app can join a card.")')
        else:
        ...
        while time.monotonic() < deadline:
            ...
            if current and not current['floating']:
                return
            time.sleep(.04)
        raise SetupError('The app could not stay inside the card layout. Check its floating window rule, then try again.')
...
def window_choices(windows, arranging=False):
    result, seen = [], set()
    for window in windows:
        label, detail = app_name(window), clean(window.get('title') or 'Open window')
        if arranging and window.get('floating'):
            detail = 'Resizes to fit card · ' + detail
...
class Setup:
    ...
    def choose_window(self, prompt, candidates, workspace, prefix='', leading=()):
        ...
        choices = list(leading) + [Choice(c.value, prefix + c.label, c.detail) for c in window_choices(local, arranging=True)]
        ...
        while True:
            ...
            if group is None:
                ...
                return selected
            subchoices = window_choices(group, arranging=True) + [Choice('back', 'Back to all apps')]
    ...
    def prepare(self, front):
        ...
        if not candidates:
            raise SetupError('Open another app that is not already in a card or group, then start again.')
        back = self.choose_window(f'Choose an app for the back · {app_name(source)} stays on the front', candidates, workspace)
    ...
    def apply_reserved(self, selected, arrangement=None, destination=None):
        ...
        try:
            ...
            floating = arrangement.get('floating', False) if arrangement else selected[front].get('floating', False)
            if floating and state.get('floating_cards'):
                self.ipc.focused((front, 'floating'))
                if not arrangement:
                    x, y = map(int, selected[front]['at'])
                    width, height = map(int, selected[front]['size'])
                    self.ipc.call('eval', f'hl.dispatch(hl.dsp.window.resize({{window="address:{front}",x={width},y={height}}})); '
                                  f'hl.dispatch(hl.dsp.window.move({{window="address:{front}",x={x},y={y}}}))')
            if arrangement:
    ...
    def validate(self, plan, focus=True):
        ...
        for address, original in plan.windows.items():
            ...
            if (not current or not current.get('mapped', True)
                    ...
                    or (current.get('grouped') and not (card.get('native_group') and current.get('grouped') == original.get('grouped')))
                    or current.get('floating') != original.get('floating')
                    or (current.get('floating') and address != plan.candidate and not card.get('floating'))):
    ...
    def prepare(self, anchor):
        ...
        if action in ('add', 'replace'):
            ...
            if not candidates:
                ...
                raise SetupError('Open another app that is not already in a card or group, then choose Add an app to this side.')
            ...
            for desktop in sorted(directory.rglob('*.desktop')):
                ...
                try:
                    ...
                    self.apps[identifier] = DesktopApp(identifier, desktop, clean(entry['Name'], 100),
                        ...
                        entry.get('Icon', '') if '://' not in entry.get('Icon', '') else '')
...
class RecipeStore:
    ...
    def validate(recipe):
        ...
        if 'workspace' in recipe and (type(recipe['workspace']) is not int or not 1 <= recipe['workspace'] <= 2147483647): raise ValueError()
        if 'floating' in recipe and type(recipe['floating']) is not bool: raise ValueError()
...
class Saved(Setup):
    ...
    def capture(card, windows):
        ...
        recipe = {'faces': faces, 'active': card['active']}
        if card.get('floating'): recipe['floating'] = True
        return recipe
    ...
    def repair_reserved(self, plan, selected, arrangement):
        ...
        def guard():
            ...
            if any(a not in live or live[a]['pid'] != w['pid'] or live[a]['class'] != w['class'] or
                   not live[a].get('mapped', True) or any(live[a].get(k) for k in ('pinned','fullscreen')) or
                   (live[a].get('floating') and not (card.get('floating') and any(a in face for face in expected))) or
                   (live[a].get('grouped') and not (card.get('native_group') and any(a in face for face in expected))) or
        ...
        while True:
            ...
            for index, name in enumerate(names):
                ...
                if opened:
                    ...
                    if any(w.get('floating') for w in matches): detail += ' · Resize apps to fit'
                if recipe.get('workspace'): detail = 'Open on workspace ' + str(recipe['workspace'])
    ...
    def prepare_manage(self, edit=None):
        ...
        choices = [Choice('open', 'Open on workspace ' + str(recipe['workspace']) if recipe.get('workspace') else
                          'Go to open card' if opened else 'Open here', self.describe(recipe))]
        if edit:
        ...
        choices += [Choice('workspace', 'Workspace…', 'Always open on workspace ' + str(recipe['workspace']) if recipe.get('workspace') else 'Open on the current workspace'),
                    Choice('rename', 'Rename…', 'Change the saved name'),
        ...
        if action == 'workspace':
            choice = self.menu.choose(f'Workspace for “{name}”', [
                Choice('current', 'Current workspace', 'Open where you are'),
                Choice('number', 'Always use a workspace…', 'Open or bring this card there')])
            updated = deepcopy(recipe)
            if choice == 'current': updated.pop('workspace', None)
            elif choice == 'number':
                number = self.menu.input('Workspace number (e.g. 3)').strip()
                if not re.fullmatch(r'[1-9][0-9]{0,9}', number) or int(number) > 2147483647:
                    raise SetupError('Enter a workspace number between 1 and 2147483647.')
                updated['workspace'] = int(number)
            else: raise Cancelled()
            return SavedPlan('settings', name, updated, previous=recipe)
    ...
    def prepare_open(self, name, recipe, workspace, focus, review=False, edit=None, chosen=None):
        ...
        # Choosing an app or confirming the arrangement authorizes fitting its
        # windows into the card; conversion and rollback happen during apply.
    ...
    def apply_open(self, plan, timeout=20):
        ...
        with exclusive():
            ...
            if plan.action != 'repair': plan = self.destination(plan)
        baseline = set(windows)
        catalog = DesktopApps(getattr(self.ipc, 'env', None))
        labels = [apps[i]['label'] for i in plan.launchers]
        progress = (self.menu.opening(plan.name, labels) if hasattr(self.menu, 'opening')
                    else Opening(plan.name, labels, getattr(self.ipc, 'env', None)))
            ...
            with progress if plan.launchers else nullcontext():
                ...
                while True:
                    ...
                    if hasattr(progress, 'update'):
                        progress.update([app['label'] for app, address in zip(apps, chosen) if not address])
    ...
    def destination(self, plan):
        """Navigate only after selections and stale-context checks have completed."""
        destination = plan.recipe.get('workspace', plan.workspace)
    ...
    def apply(self, plan):
        if plan.action == 'settings':
            self.store.update(plan.name, plan.recipe, plan.previous)
            return (f'“{plan.name}” will open on workspace {plan.recipe["workspace"]}.' if plan.recipe.get('workspace')
                    else f'“{plan.name}” will open on the current workspace.')
        if plan.action in ('save', 'update'):
            ...
            if (plan.previous or {}).get('workspace'): recipe['workspace'] = plan.previous['workspace']
            catalog = DesktopApps(getattr(self.ipc, 'env', None))
        ...
        if plan.action == 'goto':
            ...
            if plan.recipe.get('workspace') and windows[card['current']]['workspace']['id'] != plan.recipe['workspace']:
                self.ipc.focused((card['current'], 'workspace ' + str(plan.recipe['workspace'])))
            return
        if plan.action != 'restore': raise SetupError('Choose a saved card action first.')
        self.check_plan(plan)
        plan = self.destination(plan)
        original_focus = self.ipc.windows().get(plan.focus)
